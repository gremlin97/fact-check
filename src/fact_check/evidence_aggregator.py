...
class EvidenceAggregator:
    ...
    def _apply_filters(self, evidence_list: List[Dict[str, Any]], explanation: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Early return if no assessments are available
        if not assessments:
            logger.warning("No evidence assessments found. Skipping filtering.")
    ...
    def _apply_weights_and_aggregate(self, evidence_list: List[Dict[str, Any]], 
        ...
        # Create a map of evidence numbers to their assessments
        assessment_map = {}
        for assessment in assessments:
            evidence_num = assessment.get("evidence_number", 0)
            assessment_map[evidence_num] = assessment
        
        # Process each evidence item
        weighted_evidence = []
        total_score = 0.0
        
        # Map filtered evidence back to original evidence numbers
        # This is necessary since filtering changes the order
        for i, evidence in enumerate(evidence_list):
            # Look for original evidence number in paragraph_index
            paragraph_index = evidence.get("paragraph_index", -1)
            
            # Try to find the corresponding assessment by matching paragraph index
            assessment = None
            for a in assessments:
                # Match by paragraph index to ensure correct pairing after filtering
                if a.get("paragraph_index") == paragraph_index:
                    assessment = a
                    break
            
            if not assessment:
                logger.warning(f"No assessment found for evidence with paragraph_index {paragraph_index}")
                continue
            ...
            # Track the weighted evidence
            weighted_item = {
                "evidence_number": evidence_num,  # Use original evidence number
            ...
            logger.info(f"Evidence {evidence_num}: {assessment_type} with relevancy {relevancy_score} → weight {weight}")
